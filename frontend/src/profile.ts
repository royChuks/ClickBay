...
function getUserStats(userId: string): { searches: number; savedItems: number } {
...
function loadRecentActivity(): void {
...
function addWelcomeActivity(): void {
    const user = getCurrentUser();
    if (!user) return;
...
function getStoredActivities(): any[] {
    try {
...
export function updateUserStats(userId: string, action: 'search' | 'save'): void {
