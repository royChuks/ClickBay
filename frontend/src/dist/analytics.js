...
function initializeDashboard() {
    ...
    urlInput = document.getElementById('analyzeUrl');
...
function getUrlType(url) {
    if (url.includes('tech') || url.includes('techcrunch'))
        return 'tech';
    if (url.includes('blog') || url.includes('medium'))
        return 'blog';
    if (url.includes('social') || url.includes('twitter') || url.includes('trending'))
        return 'social';
    return 'tech';
}
...
function updateSentiment(sentiment) {
    ...
    if (breakdowns.length >= 3) {
        const bar0 = breakdowns[0]?.querySelector('.breakdown-bar');
        const val0 = breakdowns[0]?.querySelector('.breakdown-value');
        if (bar0)
            bar0.style.width = `${sentiment.positive}%`;
        if (val0)
            val0.textContent = `${sentiment.positive}%`;
        const bar1 = breakdowns[1]?.querySelector('.breakdown-bar');
        const val1 = breakdowns[1]?.querySelector('.breakdown-value');
        if (bar1)
            bar1.style.width = `${sentiment.neutral}%`;
        if (val1)
            val1.textContent = `${sentiment.neutral}%`;
        const bar2 = breakdowns[2]?.querySelector('.breakdown-bar');
        const val2 = breakdowns[2]?.querySelector('.breakdown-value');
        if (bar2)
            bar2.style.width = `${sentiment.negative}%`;
        if (val2)
            val2.textContent = `${sentiment.negative}%`;
    }
}
function updateKeywords(keywords) {
    ...
    if (!cloud)
        return;
    cloud.innerHTML = keywords.map(k => `<span class="keyword-bubble size-${k.size}">${k.word}</span>`).join('');
}
function updateEntities(entities) {
    const list = document.getElementById('entityList');
    if (!list)
        return;
    const activeTabEl = document.querySelector('.entity-tab.active');
    const activeTab = activeTabEl?.dataset?.type || 'people';
    const entityData = entities[activeTab] ?? entities.people;
