...
function togglePopup(event) {
    ...
    if (!navLink)
        return;
    const popup = navLink.querySelector('.popup');
    if (!popup)
        return;
...
function handlePopupOption(event) {
    const target = event.target;
    const option = target.closest('.popup-option');
    if (!option)
        return;
...
function handleNotesItemClick(event) {
    const target = event.target;
    const item = target.closest('.notes-item');
    if (!item)
        return;
...
function handleSearch(event) {
    event.preventDefault();
    const searchInput = document.querySelector('.ai-search-box');
...
function updateProfilePopup() {
    ...
    if (currentUser) {
        ...
        if (profileEmail)
            profileEmail.textContent = 'Click to login';
        if (profileContent)
...
function updateSyncStatus() {
    ...
    if (syncStatus) {
        ...
        if (syncText)
            syncText.textContent = 'Sync On';
        if (profileAvatar)
            profileAvatar.classList.add('sync-on');
    }
    else {
        if (syncIcon) {
        ...
        if (profileAvatar)
            profileAvatar.classList.remove('sync-on');
    }
}
